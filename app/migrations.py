...
def _m1(conn):
    ...
    CREATE INDEX IF NOT EXISTS idx_enrollment_token ON enrollment_tokens(token_hash);
    CREATE INDEX IF NOT EXISTS idx_routers_vpn ON routers(vpn_ip);
    """)
...
def _m2(conn):
    ...
    CREATE INDEX IF NOT EXISTS idx_access_history_router_time ON router_access_history(router_id,checked_at DESC);
...
def _m6(conn):
    """Persistent read-only AI analysis queue and report history."""
    conn.executescript("""
