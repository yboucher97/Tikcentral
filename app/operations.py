...
def register(app, page_func):
    ...
    def operations_router(router_id: int, request: Request):
        ...
        body = f'''<div class="panel pad" data-router-hero><h2>{html.escape(router['site_name'])}</h2><div class="muted">{html.escape(router['model'] or '')} · <code>{html.escape(router['vpn_ip'])}</code> · capability: {html.escape(cap['mode'] if cap else 'tikcentral_only')}</div><div class="inline" style="margin-top:12px"><a href="/timeline/{router_id}"><button>Full timeline</button></a><a href="/timeline/{router_id}/before"><button>What changed before failure?</button></a><a href="/incidents/{router_id}"><button>Build incident</button></a><a href="/compliance/{router_id}"><button>Compliance</button></a><a href="/lte/{router_id}"><button>LTE</button></a><a href="/interfaces/{router_id}"><button>Interfaces</button></a><a href="/site/{router_id}"><button>Site / customer</button></a><a href="/diagnostics/{router_id}"><button>Safe diagnostics</button></a><a href="/protection/{router_id}"><button>Protected objects</button></a><a href="/recovery/{router_id}"><button>Recovery</button></a><a href="/lifecycle/{router_id}"><button>Lifecycle</button></a><a href="/maintenance-history/{router_id}"><button>Maintenance history</button></a><a href="/hardware/{router_id}"><button>Hardware</button></a><a href="/certificates/{router_id}"><button>Certificates</button></a><a href="/change-calendar"><button>Calendar</button></a><a href="/security-audit/{router_id}"><button>Security exposure</button></a><a href="/automation-inventory/{router_id}"><button>RouterOS automation</button></a><a href="/traffic/{router_id}"><button>Traffic</button></a><a href="/capacity/{router_id}"><button>Capacity trends</button></a><a href="/notes/{router_id}"><button>Operator notes</button></a><a href="/customer-report/{router_id}"><button>Customer report</button></a><a href="/topology/{router_id}"><button>Topology</button></a><a href="/wan-probe/{router_id}"><button>WAN probe</button></a><a href="/desired-state/{router_id}"><button>Desired state</button></a><a href="/replacements?source={router_id}"><button>Replace router</button></a><a href="/alerts"><button>Alert queue</button></a><a href="/commissioning-checklist/{router_id}"><button>Commissioning checklist</button></a><a href="/time-health/{router_id}"><button>Time / NTP</button></a><a href="/mtu/{router_id}"><button>MTU / MSS</button></a><a href="/public-ip-analysis/{router_id}"><button>Public IP churn</button></a><a href="/local-utilization/{router_id}"><button>Local utilization</button></a><a href="/log-patterns/{router_id}"><button>Log patterns</button></a><a href="/model-capabilities"><button>Model capabilities</button></a><a href="/identity-collisions"><button>Identity collisions</button></a><a href="/hardware-lifecycle/{router_id}"><button>Hardware lifecycle</button></a><a href="/network-quality/{router_id}"><button>Network quality</button></a><a href="/cross-site-anomalies"><button>Cross-site anomalies</button></a></div></div>
<div class="panel pad" data-workspace-section="Summary"><h3>Lifecycle</h3><div>{html.escape(router["lifecycle_state"] or "production").title()}</div><div class="muted">{html.escape(router["lifecycle_updated_at"] or "")} {html.escape(router["lifecycle_updated_by"] or "")}</div></div>
<div class="panel pad" data-workspace-section="Summary"><h3>Site / customer</h3><div>{html.escape(site_text)}</div></div>
<div class="panel pad" data-workspace-section="Summary"><h3>Outage domain</h3><div>{html.escape(outage_text)}</div><div class="muted">{html.escape(outage["evidence"] if outage else "")}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Interface health</h3><div>{html.escape(interface_text)}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>Security exposure</h3><div>{html.escape(security_text)}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>RouterOS automation</h3><div>{html.escape(automation_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Traffic</h3><div>{html.escape(traffic_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Capacity trends</h3><div>{html.escape(capacity_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Topology</h3><div>{html.escape(topology_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>WAN probe</h3><div>{html.escape(wan_probe_text)}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>Desired state</h3><div>{html.escape(desired_text)}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>Commissioning checklist</h3><div>{html.escape(checklist_text)}</div><div style="margin-top:10px"><a href="/commissioning-checklist/{router_id}"><button>Open checklist</button></a></div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Time / NTP</h3><div>{html.escape(time_health_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>MTU / MSS</h3><div>{html.escape(mtu_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Public IP churn</h3><div>{html.escape(public_ip_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Local network utilization</h3><div>{html.escape(local_utilization_text)}</div></div>
<div class="panel pad" data-workspace-section="Assets"><h3>Model capabilities</h3><div>{html.escape(model_capability_text)}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>Router log patterns</h3><div>{html.escape(log_pattern_text)}</div></div>
<div class="panel pad" data-workspace-section="Assets"><h3>Hardware lifecycle</h3><div>{html.escape(hardware_lifecycle_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>Network quality</h3><div>{html.escape(network_quality_text)}</div><div class="muted">DNS: {html.escape(dns_quality_text)} · Gateway: {html.escape(gateway_quality_text)}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>Golden policy</h3><div>{html.escape(compliance_text)}</div></div>
<div class="panel pad" data-workspace-section="Connectivity"><h3>LTE</h3><div>{html.escape(lte_text)}</div></div>
<div class="panel pad" data-workspace-section="Summary"><h3>Access / commissioning</h3><div>{'Healthy' if access and access['management_ok'] else 'Degraded'} · commissioning {html.escape(commissioning)} · config {html.escape(drift_status)}</div><div class="inline" style="margin-top:12px">{_post_button(f'/operations/{router_id}/commission',csrf,'Run commissioning validation')} {_post_button(f'/operations/{router_id}/baseline',csrf,'Accept current baseline')} {_post_button(f'/operations/{router_id}/drift/check',csrf,'Check drift')}</div></div>
<div class="panel pad" data-workspace-section="Summary"><h3>Telemetry</h3><div>{telemetry_text}</div><div class="inline" style="margin-top:12px">{_post_button(f'/operations/{router_id}/telemetry',csrf,'Refresh telemetry')} {_post_button(f'/operations/{router_id}/backup/pre-change',csrf,'Take retained backup')}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>Performance</h3><div>Current: {html.escape(profile if cap and cap['supports_performance_profiles'] else 'N/A')}</div><div class="inline" style="margin-top:12px">{profile_buttons or '<span class="muted">Performance profiles are not enabled for this router capability.</span>'}</div></div>
<div class="panel pad" data-workspace-section="Configuration"><h3>RouterOS / RouterBOOT</h3><div class="inline">{upgrade_buttons} {approve}</div></div>
<div class="panel"><div class="pad tc-table-heading"><h3>Recent backups</h3></div><table><thead><tr><th>Backup tier</th><th>Time</th><th>By</th><th>Result</th></tr></thead><tbody>{backup_rows}</tbody></table></div>
<div class="panel"><div class="pad tc-table-heading"><h3>Recent jobs</h3></div><table><thead><tr><th>Time</th><th>Job</th><th>Status</th><th>Target</th><th>Error</th></tr></thead><tbody>{job_rows}</tbody></table></div>
...
<div class="panel"><div class="pad tc-table-heading"><h3>Recent events</h3></div><table><thead><tr><th>Time</th><th>Severity</th><th>Category</th><th>Event</th></tr></thead><tbody>{event_rows}</tbody></table></div>'''
