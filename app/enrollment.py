"""Single enrollment/provisioning web module."""
...
def _page(page_func, user, csrf, message=""):
    ...
    note = f'<div class="panel pad"><strong>{html.escape(message)}</strong></div>' if message else ""
...
<label>Performance profile<br><select name="performance_profile" style="width:100%"><option value="throughput" selected>Maximum throughput</option><option value="fairness">Fairness / QoS</option></select></label>
...
<div class="panel pad" style="margin-top:16px"><strong>Profile behavior</strong><div class="muted" style="margin-top:6px"><b>Always active:</b> conservative RAW prefiltering and TCP MSS clamp-to-PMTU.<br><b>Maximum throughput:</b> FastTrack active; CAKE, QoS mangle and tenant queues are staged but disabled.<br><b>Fairness / QoS:</b> FastTrack disabled; CAKE shaping, QoS classification and tenant caps enabled.</div></div>
<div class="muted" style="margin-top:12px">Fresh config creates DHCP on ether1 plus Bell VLAN35 and a disabled dummy PPPoE profile. PPPoE distance 1; DHCP fallback distance 2.</div>
...
<div class="muted">Saved once and reconciled on generated scripts. Password is encrypted at rest. Current status: <strong>{'Configured' if configured else 'Not configured'}</strong>.</div>
...
def register(app, page_func):
    ...
    async def save_admin_credentials(request: Request):
        ...
        try:
            provisioning.set_admin_credentials(username, password)
    ...
    async def generate(request: Request):
        ...
        try:
            ...
            lan_count = int(data.get("lan_count", "4"))
            wan_down = int(data.get("wan_down", "500"))
            wan_up = int(data.get("wan_up", "500"))
            tenant_down = int(data.get("tenant_down", "80"))
            tenant_up = int(data.get("tenant_up", "40"))
        except ValueError:
            raise HTTPException(status_code=400, detail="invalid numeric provisioning value")
        vlan_parent = data.get("vlan_parent", "ether2")
        valid = (
            1 <= vlan_count <= 60 and 0 <= lan_count <= 4 and
            10 <= wan_down <= 10000 and 10 <= wan_up <= 10000 and
            0 <= tenant_down <= 10000 and 0 <= tenant_up <= 10000 and
            vlan_parent in interface_choices.INTERFACE_SET
        )
        if not valid:
        ...
        with core.db() as conn:
            conn.execute(
                "INSERT INTO enrollment_tokens(token_hash,site_name,created_at,expires_at,provision_mode,performance_profile) VALUES(?,?,?,?,?,?)",
                (core.hash_token(token), site_name, core.iso(now), core.iso(expires), mode, profile),
            )

        parts = []
        if mode == "default":
            base = performance_profile.performance_ready_default_config_script(
                site_name, vlan_count, lan_count, wan_down, wan_up,
                tenant_down, tenant_up, vlan_parent,
            )
            parts.append(performance_profile.apply_profile(base, profile))
        parts.append(management_script.build_routeros_script(site_name, token))
        ...
        summary = "Tikcentral only" if mode == "enroll" else (
            f"Opticable default config + Tikcentral · {vlan_count} VLANs · {lan_count} LANs · "
            f"{wan_down}/{wan_up} Mbps · {performance_profile.PROFILE_LABELS[profile]}"
        )
        ...
        body = f'''{warning}<div class="panel pad"><h2>{html.escape(site_name)}</h2><div>{html.escape(summary)}</div><div class="muted">One-time token expires {html.escape(expiry_local)} (Montréal time). {admin_note}</div><div class="inline" style="margin-top:14px"><button type="button" class="primary" onclick="tcCopy(document.getElementById('script'),this)">Copy script</button><a href="/enroll"><button type="button">Back</button></a></div></div><div class="panel pad"><textarea class="script" id="script" readonly>{html.escape(script)}</textarea></div>'''
