...
(function(){
 ...
 function filterTable(wrap){const q=(wrap.querySelector('.tc-local-search')?.value||'').toLowerCase();let shown=0,total=0;wrap.querySelectorAll('tbody tr').forEach(r=>{total++;const ok=!q||(r.innerText||'').toLowerCase().includes(q);r.style.display=ok?'':'none';if(ok)shown++});const c=wrap.querySelector('.tc-count');if(c)c.textContent=shown+' of '+total+' rows'}
 function enhanceTable(table,index){if(table.dataset.tcReady)return;table.dataset.tcReady='1';const panel=table.parentElement;const wrap=document.createElement('div');wrap.className='tc-table-wrap';panel.insertBefore(wrap,table);const tools=document.createElement('div');tools.className='tc-table-tools';tools.innerHTML='<input class="tc-local-search" placeholder="Search this table…"><span class="tc-count"></span><div class="tc-colbox"><button type="button" class="tc-colbtn">Columns ▾</button><div class="tc-colmenu"></div></div><button type="button" class="tc-reset">Reset view</button>';wrap.appendChild(tools);const scroll=document.createElement('div');scroll.className='tc-scroll';wrap.appendChild(scroll);scroll.appendChild(table);const key='tikcentral:columns:'+location.pathname+':'+index;let hidden=[];try{hidden=JSON.parse(localStorage.getItem(key)||'[]')}catch(_){hidden=[]}const headers=[...table.querySelectorAll('thead th')];const menu=tools.querySelector('.tc-colmenu');headers.forEach((th,i)=>{th.dataset.sort='1';let dir=1;th.addEventListener('click',e=>{if(e.target.closest('input,button'))return;sortTable(table,i,dir);dir*=-1});const label=(th.innerText||('Column '+(i+1))).trim()||('Column '+(i+1));const row=document.createElement('label');row.innerHTML='<input type="checkbox" '+(hidden.includes(i)?'':'checked')+'><span></span>';row.querySelector('span').textContent=label;const cb=row.querySelector('input');cb.addEventListener('change',()=>{document.querySelectorAll('');setCol(i,cb.checked)});menu.appendChild(row);setCol(i,!hidden.includes(i),false)});function setCol(i,show,save=true){[...table.rows].forEach(r=>{if(r.cells[i])r.cells[i].style.display=show?'':'none'});hidden=hidden.filter(x=>x!==i);if(!show)hidden.push(i);if(save)localStorage.setItem(key,JSON.stringify(hidden))}tools.querySelector('.tc-colbtn').onclick=e=>{e.stopPropagation();tools.querySelector('.tc-colbox').classList.toggle('open')};tools.querySelector('.tc-local-search').oninput=()=>filterTable(wrap);tools.querySelector('.tc-reset').onclick=()=>{tools.querySelector('.tc-local-search').value='';hidden=[];localStorage.removeItem(key);headers.forEach((_,i)=>setCol(i,true,false));menu.querySelectorAll('input').forEach(x=>x.checked=true);filterTable(wrap)};filterTable(wrap)}
 ...
 document.addEventListener('DOMContentLoaded',()=>{themeLabel();document.querySelectorAll('table').forEach(enhanceTable);const g=document.getElementById('tcGlobalSearch');if(g)g.addEventListener('input',globalFilter);document.querySelectorAll('td').forEach(td=>{if(td.children.length)return;const s=(td.innerText||'').trim().toLowerCase();let tone='';if(['healthy','online','passed','success','succeeded','enabled','ready','commissioned','matches baseline'].includes(s))tone='ok';else if(['warning','partial','degraded','pending','queued','running','verifying','drift','drift detected'].includes(s))tone='warn';else if(['failed','error','critical','offline'].includes(s))tone='bad';if(tone){const text=td.innerText;td.innerHTML='<span class="tc-status '+tone+'"></span>';td.firstChild.textContent=text}})});
})();
'''
